...
| P1-BASE-010 | API 客户端生成 | [F] | 基于 Django Ninja 的 OpenAPI Schema，使用 openapi-typescript-codegen 生成类型安全的 API 客户端 | P0 | 3h | 004 | ❌ 未完成（可用替代方案：直接使用 axios，类型可后续补充） | A |
...
| P1-AUTH-003 | GitHub OAuth | [B] | 配置 django-allauth GitHub Provider，返回 JWT | P1 | 2h | AUTH-001 | ❌ 未完成 | A |
| P1-AUTH-004 | Google OAuth | [B] | 配置 django-allauth Google Provider，返回 JWT | P1 | 2h | AUTH-001 | ❌ 未完成 | A |
| P1-AUTH-005 | 邮箱验证 | [B] | django-allauth 内置邮箱验证流程 + 自定义模板 | P1 | 2h | AUTH-002 | ❌ 未完成 | A |
| P1-AUTH-006 | 忘记密码/重置密码 | [B] | 重置密码 API + 邮件发送 | P1 | 3h | AUTH-002 | ❌ 未完成 | A |
...
| P1-USER-003 | 公开用户页 | [F] | /u/:username 公开资料页，展示统计数据和贡献 | P1 | 3h | USER-001 | ❌ 未完成 | A |
...
| P1-USER-006 | 信用分历史页 | [F] | 信用分变动记录列表 | P1 | 2h | USER-004 | ❌ 未完成 | A |
| P1-USER-007 | 头像上传 | [B+F] | 后端：Django 文件上传 API（django-storages + S3/R2）；前端：上传组件 + 裁剪 | P1 | 3h | USER-001 | ❌ 未完成 | A |
| P1-USER-008 | 通知系统后端 | [B] | NotificationService：send, mark_read, list（Django 服务层） | P1 | 3h | BASE-003 | ❌ 未完成 | A |
| P1-USER-009 | 通知系统前端 | [F] | NotificationBell 组件 + 通知列表页 + SSE 实时推送 | P1 | 4h | USER-008, BASE-010 | ❌ 未完成 | A |
...
| 任务ID | 任务名称 | 端 | 描述 | 优先级 | 预估 | 依赖 | 状态 | 负责人 |
|--------|---------|-----|------|--------|------|------|------|--------|
| P1-INV-001 | 邀请码生成与验证 | [B] | 生成唯一邀请码，注册时绑定邀请关系 | P1 | 3h | AUTH-002 | ❌ 未完成 | A |
| P1-INV-002 | 邀请奖励发放 | [B] | 注册奖励（即时）+ 首充奖励（延迟）+ 消费奖励（延迟），使用 Celery 异步处理 | P1 | 4h | INV-001, USER-004 | ❌ 未完成 | A |
| P1-INV-003 | 邀请页面 | [F] | 邀请码展示 + 邀请统计 + 分享链接 | P1 | 3h | INV-001, BASE-010 | ❌ 未完成 | A |
| P1-INV-004 | 反刷机制 | [B] | IP/设备检测 + 7天活跃校验 + 月度上限 | P2 | 3h | INV-001 | ❌ 未完成 | A |
...
|--------|---------|-----|------|--------|------|------|--------|
| P2-PAY-001 | Stripe 集成 | [B] | 使用 stripe Python SDK，配置 Stripe 密钥和产品 | P0 | 4h | BASE-003 | A |
| P2-PAY-002 | 充值流程 | [B+F] | create_deposit_session → Stripe Checkout → Django Webhook 视图 → Celery 异步余额到账 | P0 | 6h | PAY-001 | A |
| P2-PAY-003 | Transaction 服务 | [B] | TransactionService（Django 服务层）：记录交易流水，查询余额，生成报表 | P0 | 4h | BASE-003 | A |
| P2-PAY-004 | 钱包页面 | [F] | 余额展示 + 充值入口 + 交易记录 + 收入报表 | P0 | 5h | PAY-002, PAY-003, BASE-010 | A |
| P2-PAY-005 | 余额组件 | [F] | 导航栏余额展示 + 充值快捷入口 | P1 | 2h | PAY-003, BASE-010 | A |
...
| 任务ID | 任务名称 | 端 | 描述 | 优先级 | 预估 | 依赖 | 负责人 |
|--------|---------|-----|------|--------|------|------|--------|
| P2-TIP-001 | 打赏后端逻辑 | [B] | tip API：余额扣减 → 转账 → 记录 → 信用分（transaction.atomic） | P0 | 3h | PAY-003 | A |
| P2-TIP-002 | 打赏 UI | [F] | TipDialog 组件：快捷金额 + 自定义 + 确认 | P0 | 3h | TIP-001, BASE-010 | A |
| P2-TIP-003 | 打赏排行榜 | [B+F] | 按累计收到打赏排序 + 每周更新 + Redis 缓存 | P1 | 3h | TIP-001 | A |
| P2-TIP-004 | 打赏记录展示 | [F] | 文章详情页显示打赏记录 | P1 | 2h | TIP-001, BASE-010 | A |
...
| 任务ID | 任务名称 | 端 | 描述 | 优先级 | 预估 | 依赖 | 负责人 |
|--------|---------|-----|------|--------|------|------|--------|
| P3-CREDIT-001 | API 倍率折扣 | [B] | Skill 调用时根据等级计算折扣价格 | P0 | 3h | P1-USER-004 | A |
| P3-CREDIT-002 | 等级门槛校验 | [B] | 悬赏发布/接单的信用分门槛检查 | P0 | 2h | P1-USER-004 | A |
| P3-CREDIT-003 | 等级可视化增强 | [F] | 头像等级框、特效、VIP 标识 | P1 | 4h | P1-USER-005 | A |
| P3-CREDIT-004 | 悬赏板冻结逻辑 | [B] | 信用分 < 30 自动冻结 + 恢复检查 | P1 | 2h | P1-USER-004 | A |

### Sprint 3.2：管理后台（第8~9周）

| 任务ID | 任务名称 | 端 | 描述 | 优先级 | 预估 | 依赖 | 负责人 |
|--------|---------|-----|------|--------|------|------|--------|
| P3-ADMIN-001 | Admin 布局 | [F] | 管理后台侧边栏布局 + 权限路由守卫 | P0 | 3h | P1-AUTH-008 | A |
| P3-ADMIN-002 | 仪表盘 | [B+F] | 平台概览数据 API + 前端展示：用户数、Skill数、文章数、流水额（recharts 图表） | P0 | 4h | P2-PAY-003 | A |
| P3-ADMIN-003 | Skill 审核页 | [B+F] | 后端审核 API + 前端：待审核列表 + 审核操作（通过/拒绝 + 理由）+ 预览 | P0 | 5h | P1-SKILL-008 | A |
| P3-ADMIN-004 | 用户管理页 | [B+F] | 后端用户管理 API + 前端：用户列表 + 搜索 + 封禁/解封 + 角色调整 | P0 | 4h | P1-USER-001 | A |
...
| P3-ADMIN-008 | 财务管理页 | [B+F] | 财务统计 API + 前端：充值统计 + 手续费收入 + $ 流通量 + 趋势图 | P1 | 5h | P2-PAY-003 | A |
...
| 任务ID | 任务名称 | 端 | 描述 | 优先级 | 预估 | 依赖 | 负责人 |
|--------|---------|-----|------|--------|------|------|--------|
| P3-RANK-001 | 信用分排行榜 | [B] | Top 50 用户 + Redis Sorted Set + Celery Beat 定时更新 | P1 | 3h | P1-USER-004 | A |
| P3-RANK-002 | Skill 热门榜 | [B] | 近7天调用量 x 评分排序 + 每小时刷新（Celery Beat） | P1 | 3h | P2-SKILL-006 | A |
| P3-RANK-003 | 创作者排行榜 | [B] | 按总收入/总调用/信用分排序 | P2 | 3h | P2-SKILL-002 | A |
| P3-RANK-004 | 排行榜页面 | [F] | 统一排行榜展示页面（Tab切换不同榜单） | P1 | 4h | RANK-001~003, BASE-010 | A |
